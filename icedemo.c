...
static void UDPCreateThread(){
  int rt;
  pthread_t id;
  rt=pthread_create(&id,NULL,UDPRecvThread,NULL);
  if(rt!=0){
...
static void  UDPsend(void*data, int data_len){
   int sent;
   sent=write(icedemo.localfd,data,data_len);
 
   if(sent==-1){
    perror("WRITE");
    }
    printf("data to WRITE:%d,data WRITE:%d\n",data_len,sent); 
}
			       ...
			       pj_status_t status)
{
	...
	(op==PJ_ICE_STRANS_OP_INIT? "initialization" :
        ...
        /* when the initialization complete,we can get the local candidates */
    if(status==PJ_SUCCESS && op==PJ_ICE_STRANS_OP_INIT ){
        PJ_LOG(4,(THIS_FILE,"!!<<<<<<<<<<<<<<<<<<<<<ice init complete"));
        //icedemo_init_session(1);
        //icedemo_show_ice(); 
    }
   ...
   if(status==PJ_SUCCESS && op==PJ_ICE_STRANS_OP_NEGOTIATION){     
     PJ_LOG(4,(THIS_FILE,"!!>>>>>>>>>>>>> NEGOTIATION COMPLETE! THEN  GET PAIR"));
     sess_check=pj_ice_strans_get_valid_pair(ice_st,1);
     lcand=sess_check->lcand;
     rcand=sess_check->rcand;
     pj_sockaddr_print(&rcand->addr,icedemo.ripaddr,sizeof(icedemo.ripaddr),0);
     icedemo.rport=(int)pj_sockaddr_get_port(&rcand->addr);
     printf("REMOTE--> port:%d,IP:%s\n",icedemo.rport,icedemo.ripaddr);
    
 
     pj_sockaddr_print(&lcand->addr,icedemo.lipaddr,sizeof(icedemo.lipaddr),0);
     icedemo.lport=(int)pj_sockaddr_get_port(&lcand->addr);
     printf("LOCAL--> port:%d,IP:%s\n",icedemo.lport,icedemo.lipaddr);
...
static void icedemo_send_data(unsigned comp_id, const char *data)
{
   ...
   /*
     PJ_LOG(3,(THIS_FILE,">>>>>>>>>>>>>>being UDPsend!!"));
     UDPsend((void*)data,strlen(data));
    sock=(icedemo.icest->ice->comp[0])->stun_sock->active_sock->key->fd;
    ice=icedemo.icest->ice;
    ice_st = (pj_ice_strans*)ice->user_data;
    comp=(icedemo.icest)->comp[0]; 
    stun_sock=comp->stun_sock;
    active_sock=stun_sock->active_sock;
    key=active_sock->key;
    sock=key->fd;
